/**
 * Der Prüfstand — die Messungen.
 *
 * Portiert aus `pawn-prototype/tools/pruefstand/messen.ts`. Jede Funktion misst
 * genau eine ZERA-Kontrolle und gibt Befunde zurück. Ein Befund trägt immer
 * `gemessen` UND `schwelle`. Es wird nichts bewertet, nichts formuliert, nichts
 * geraten — nur gemessen und aufgeschrieben.
 *
 * WAS BEIM PORTIEREN WEGGEFALLEN IST, UND WARUM:
 *
 *   messeDrehHinweis    — X.dreh, der Hinweis „bitte drehen" auf den Heftseiten
 *   messeBlattgeometrie — X.blatt, das Seitenverhältnis 0,72 einer Doppelseite
 *
 * Beide messen Bauteile, die es nur in PAWN gibt. Mitgeschleppt hätten sie auf
 * jeder Breite `nicht_pruefbar` gemeldet — drei Zeilen Rauschen je Lauf, die
 * aussehen wie ein Befund und keiner sind. Ein Messgerät, das dauernd über
 * Dinge redet, die es nicht gibt, erzieht dazu, es zu überlesen.
 *
 * Geblieben ist alles, was über JEDE Seite etwas sagt: 3.3 Kontrast,
 * 3.4 Fokus, 3.5 Trefferflächen, 3.8 Layout, 3.10 der primäre Knopf,
 * 5.1/5.2/5.3/5.4 die Kopfdaten.
 */
...
export async function messeFokus(page: Page, seite: string, breite: number): Promise<Befund[]> {
  const stellen = await page.evaluate((sel) => {
    ...
    for (const el of Array.from(document.querySelectorAll(sel))) {
      ...
      // niemand fokussieren kann (z. B. eine Schicht, die gerade geschlossen ist).
  ...
  if (!breite.trefferflaechen) {
    return [{
      ...
      notiz: breite.eingabe === "maus"
        ...
        : `${breite.name}: Hochformat auf dem Telefon — gemessen wird die Lage, in der wirklich mit dem Finger bedient wird (quer).`,
    }];
  }
